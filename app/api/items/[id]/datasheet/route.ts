...
export async function GET(
  ...
  try {
    ...
    if (!item.datasheetURL)
      return NextResponse.json({ error: "Item has no datasheet" }, { status: 404 });

    const key = item.datasheetURL;
    ...
    console.error("Item datasheet GET failed:", err);
    return NextResponse.json(
      { error: err?.message ?? "Failed to fetch item datasheet" },
      { status: 500 }
    );
  }
}
